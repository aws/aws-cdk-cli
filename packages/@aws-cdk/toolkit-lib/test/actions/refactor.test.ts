...
describe('refactor detection', () => {
  ...
  test('computes one set of mappings per environment', async () => {
    ...
    expect(ioHost.notifySpy).toHaveBeenNthCalledWith(4,
      expect.objectContaining({
        action: 'refactor',
        level: 'result',
        code: 'CDK_TOOLKIT_I8900',
  ...
  test('bootstrap version lower than minimum required', async () => {
    ...
    expect(ioHost.notifySpy).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        code: 'CDK_TOOLKIT_E8900',
        level: 'error',
        message: "❌  The CDK toolkit stack in environment aws://123456789012/us-east-1 doesn't support refactoring. Please run 'cdk bootstrap' to update it.",
      }),
    );
  });
});
