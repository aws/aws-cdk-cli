...
import { PluginHost, Mode } from '../../../lib/api/plugin';
...
beforeEach(() => {
  ...
  pluginHost.credentialProviderSources.splice(0);
  pluginHost.credentialProviderSources.push({
    isAvailable() {
...
describe('with intercepted network calls', () => {
  ...
  describe('when CDK does not AssumeRole', () => {
    ...
    test('no base credentials partition if token is expired', async () => {
      ...
      const provider = new SdkProvider(identityProvider, 'rgn', { ioHelper, pluginHost: pluginHost });
    ...
    const providersForMfa = [
      ...
      (async () => {
        ...
        const provider = await SdkProvider.withAwsCliCompatibleDefaults({ ioHelper, logger: console, pluginHost: pluginHost });
