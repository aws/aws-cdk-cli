...
describe('toCloudFormationRollbackConfiguration', () => {
  ...
  test.each([-1, 181])('throws when monitoring time %d is out of range', (monitoringTimeInMinutes) => {
    expect(() => toCloudFormationRollbackConfiguration({ monitoringTimeInMinutes })).toThrow(/monitoring time must be a whole number between 0 and 180/);
  });

  test.each([12.5, NaN])('throws when monitoring time %d is not a whole number', (monitoringTimeInMinutes) => {
    expect(() => toCloudFormationRollbackConfiguration({ monitoringTimeInMinutes })).toThrow(/monitoring time must be a whole number between 0 and 180/);
  });
