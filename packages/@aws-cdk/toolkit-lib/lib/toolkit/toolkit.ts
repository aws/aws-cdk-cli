  ...
  private async _deploy(assembly: StackAssembly, action: 'deploy' | 'watch', options: ExtendedDeployOptions = {}) {
    ...
    const deployStack = async (stackNode: StackNode) => {
      ...
      if (Object.keys(stack.template.Resources || {}).length === 0) {
        ...
        if (!stackExists) {
          return ioHost.notify(CODES.CDK_TOOLKIT_I5021.msg(`${chalk.bold(stack.displayName)}: stack has no resources, skipping deployment.`));
        }

        // stack is empty, but exists => delete
        await ioHost.notify(CODES.CDK_TOOLKIT_I5022.msg(`${chalk.bold(stack.displayName)}: stack has no resources, deleting existing stack.`));
