...
export class Toolkit extends CloudAssemblySourceBuilder implements AsyncDisposable {
  ...
  public async bootstrap(environments: BootstrapEnvironments, options: BootstrapOptions): Promise<void> {
    ...
    await Promise.all(bootstrapEnvironments.map((environment: cxapi.Environment, currentIdx) => limit(async () => {
      await ioHost.notify(CODES.CDK_TOOLKIT_I9100.msg(`${chalk.bold(environment.name)}: bootstrapping...`, {
      ...
      try {
        ...
        await ioHost.notify(CODES.CDK_TOOLKIT_I9900.msg(chalk.green('\n' + message), { environment }));
        await bootstrapTimer.endAs(ioHost, 'bootstrap');
      } catch (e: any) {
        await ioHost.notify(CODES.CDK_TOOLKIT_E9900.msg(`\n ❌  ${chalk.bold(environment.name)} failed: ${formatErrorMessage(e)}`, { error: e }));
        throw e;
      }
    })));
  }
  ...
  public async synth(cx: ICloudAssemblySource, options: SynthOptions = {}): Promise<ICloudAssemblySource> {
    ...
    if (stacks.stackCount === 1) {
      ...
      await ioHost.notify(CODES.CDK_TOOLKIT_I1902.msg(chalk.green(message), assemblyData));
      await ioHost.notify(CODES.DEFAULT_TOOLKIT_INFO.msg(`Supply a stack id (${stacks.stackArtifacts.map((s) => chalk.green(s.hierarchicalId)).join(', ')}) to display its template.`));
    }
  ...
  private async _deploy(assembly: StackAssembly, action: 'deploy' | 'watch', options: ExtendedDeployOptions = {}) {
    ...
    if (hotswapMode !== HotswapMode.FULL_DEPLOYMENT) {
      await ioHost.notify(CODES.CDK_TOOLKIT_W5400.msg([
    ...
    const deployStack = async (stackNode: StackNode) => {
      ...
      if (stackCollection.stackCount !== 1) {
        await ioHost.notify(CODES.DEFAULT_TOOLKIT_INFO.msg(chalk.bold(stack.displayName)));
      }
      ...
      if (Object.keys(stack.template.Resources || {}).length === 0) {
        ...
        if (!stackExists) {
          return ioHost.notify(CODES.CDK_TOOLKIT_W5021.msg(`${chalk.bold(stack.displayName)}: stack has no resources, skipping deployment.`));
        }

        // stack is empty, but exists => delete
        await ioHost.notify(CODES.CDK_TOOLKIT_W5022.msg(`${chalk.bold(stack.displayName)}: stack has no resources, deleting existing stack.`));
      ...
      await ioHost.notify(CODES.CDK_TOOLKIT_I5100.msg(`${chalk.bold(stack.displayName)}: deploying... [${stackIndex}/${stackCollection.stackCount}]`, {
      ...
      try {
        ...
        await ioHost.notify(CODES.CDK_TOOLKIT_I5900.msg(chalk.green('\n' + message), deployResult));
        ...
        if (options.traceLogs) {
          ...
          await ioHost.notify(CODES.CDK_TOOLKIT_I5031.msg(`The following log groups are added: ${foundLogGroupsResult.logGroupNames}`));
        }
      ...
      await ioHost.notify(CODES.CDK_TOOLKIT_I5001.msg(`\n✨  Total time: ${formatTime(duration)}s\n`, { duration }));
    };
  ...
  public async watch(cx: ICloudAssemblySource, options: WatchOptions): Promise<void> {
    ...
    await ioHost.notify(CODES.CDK_TOOLKIT_I5310.msg([
    ...
    const deployAndWatch = async () => {
      ...
      while (latch === 'queued') {
        ...
        latch = 'deploying';
        await ioHost.notify(CODES.CDK_TOOLKIT_I5315.msg("Detected file changes during deployment. Invoking 'cdk deploy' again"));
        await this.invokeDeployFromWatch(assembly, options, cloudWatchLogMonitor);
      }
      latch = 'open';
    ...
    chokidar
      ...
      .on('ready', async () => {
        latch = 'open';
        await ioHost.notify(CODES.DEFAULT_TOOLKIT_DEBUG.msg("'watch' received the 'ready' event. From now on, all file changes will trigger a deployment"));
        await ioHost.notify(CODES.CDK_TOOLKIT_I5314.msg("Triggering initial 'cdk deploy'"));
      ...
      .on('all', async (event: 'add' | 'addDir' | 'change' | 'unlink' | 'unlinkDir', filePath: string) => {
        ...
        if (latch === 'pre-ready') {
          await ioHost.notify(CODES.CDK_TOOLKIT_I5311.msg(`'watch' is observing ${event === 'addDir' ? 'directory' : 'the file'} '${filePath}' for changes`, watchEvent));
        } else if (latch === 'open') {
          await ioHost.notify(CODES.CDK_TOOLKIT_I5312.msg(`Detected change to '${filePath}' (type: ${event}). Triggering 'cdk deploy'`, watchEvent));
          await deployAndWatch();
        } else {
          // this means latch is either 'deploying' or 'queued'
          latch = 'queued';
          await ioHost.notify(CODES.CDK_TOOLKIT_I5313.msg(
  ...
  private async _rollback(assembly: StackAssembly, action: 'rollback' | 'deploy' | 'watch', options: RollbackOptions): Promise<void> {
    ...
    if (stacks.stackCount === 0) {
      await ioHost.notify(CODES.CDK_TOOLKIT_E6001.msg('No stacks selected'));
      return;
    }
    ...
    for (const [index, stack] of stacks.stackArtifacts.entries()) {
      await ioHost.notify(CODES.CDK_TOOLKIT_I6100.msg(`Rolling back ${chalk.bold(stack.displayName)}`, {
  ...
  private async _destroy(assembly: StackAssembly, action: 'deploy' | 'destroy', options: DestroyOptions): Promise<void> {
    ...
    try {
      for (const [index, stack] of stacks.stackArtifacts.entries()) {
        await ioHost.notify(CODES.CDK_TOOLKIT_I7100.msg(chalk.green(`${chalk.blue(stack.displayName)}: destroying... [${index + 1}/${stacks.stackCount}]`), {
          total: stacks.stackCount,
          current: index + 1,
          stack,
        }));
        try {
          ...
          await ioHost.notify(CODES.CDK_TOOLKIT_I7900.msg(chalk.green(`\n ✅  ${chalk.blue(stack.displayName)}: ${action}ed`), stack));
        } catch (e: any) {
          await ioHost.notify(CODES.CDK_TOOLKIT_E7900.msg(`\n ❌  ${chalk.blue(stack.displayName)}: ${action} failed ${e}`, { error: e }));
          throw e;
        }
      }
    } finally {
  ...
  private async validateStacksMetadata(stacks: StackCollection, ioHost: ActionAwareIoHost) {
    const builder = (level: IoMessageLevel) => {
      switch (level) {
        ...
        case 'warn': return CODES.CDK_ASSEMBLY_W9999;
        default: return CODES.CDK_ASSEMBLY_I9999;
      }
    };
