...
export class EnvironmentResources {
    ...
    function doValidate(version: number, environment: Environment) {
      ...
      if (defExpectedVersion > version) {
        throw new BootstrapError(
          ...
          `This CDK deployment requires bootstrap stack version '${expectedVersion}', found '${version}' in ${environment.name}. Please run 'cdk bootstrap'.`,
  ...
  public async versionFromSsmParameter(parameterName: string): Promise<number> {
    ...
    try {
      ...
      if (isNaN(asNumber)) {
        throw new ToolkitError('SsmParameterNotANumber', `SSM parameter ${parameterName} not a number: ${result.Parameter?.Value}`);
      }
      ...
      if (e.name === 'ParameterNotFound') {
        throw new BootstrapError(
          ...
          `SSM parameter ${parameterName} not found in ${this.environment.name}. Has the environment been bootstrapped? Please run \'cdk bootstrap\' (see https://docs.aws.amazon.com/cdk/latest/guide/bootstrapping.html)`,
  ...
  public async prepareEcrRepository(repositoryName: string): Promise<EcrRepositoryInfo> {
    ...
    if (!repositoryUri) {
      throw new ToolkitError('EcrRepositoryCreateFailed', `CreateRepository did not return a repository URI for ${repositoryUri}`);
    }
