...
export async function execInChildProcess(commandAndArgs: string, options: ExecOptions = {}) {
  ...
  try {
    ...
    // user fast and the IoHost receives whole lines. A synth can produce a lot
    // of output; it is consumed via onOutput only, so don't also retain it.
    await runUserCommandLine(commandAndArgs, {
      stdio: captureOutput ? 'capture' : 'inherit',
      buffering: 'lines',
      collect: false,
      ...(captureOutput ? {
        onOutput: (stream: 'stdout' | 'stderr', line: string) => {
          if (stream === 'stderr') {
            stderr.push(line);
          }
    ...
    const failure = e.kind === 'killed' ? `was killed with signal ${e.signal}` : `exited with error ${e.exitCode}`;
    const error = AssemblyError.withCause(`${commandAndArgs}: Subprocess ${failure}`, cause);
