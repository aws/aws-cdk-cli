...
import { WorkGraphBuilder } from '../../../src/api/work-graph';
import type { AssetBuildNode, AssetPublishNode, StackNode, WorkNode, WorkGraph } from '../../../src/api/work-graph';
import { TestIoHost } from '../../_helpers/io-host';

let rootBuilder: CloudAssemblyBuilder;
let ioHelper = new TestIoHost().asHelper();
...
describe('with some stacks and assets', () => {
  ...
  test('stack depends on the asset publishing step', () => {
    const graph = new WorkGraphBuilder(ioHelper, true).build(assembly.artifacts);
  ...
  test('with prebuild off, asset building inherits dependencies from their parent stack', () => {
    const graph = new WorkGraphBuilder(ioHelper, false).build(assembly.artifacts);
...
test('dependencies on unselected artifacts are silently ignored', async () => {
  ...
  const graph = new WorkGraphBuilder(ioHelper, true).build([asm.getStackArtifact('stackB')]);
