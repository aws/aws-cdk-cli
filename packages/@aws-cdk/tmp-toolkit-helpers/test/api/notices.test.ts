...
import { asIoHelper, IoDefaultMessages } from '../../src/api/io/private';
import type {
  Notice,
...
import { FakeIoHost } from '../_helpers/io-host';
...
describe(NoticesFilter, () => {
  describe('filter', () => {
    ...
    test('framework', () => {
      ...
      expect(noticesFilter.filter({ data: notices, cliVersion, bootstrappedEnvironments: [], outDir: path.join(fixtures, 'built-with-2_12_0') }).map(f => f.notice)).toEqual([]);
      expect(noticesFilter.filter({ data: notices, cliVersion, bootstrappedEnvironments: [], outDir: path.join(fixtures, 'built-with-1_144_0') }).map(f => f.notice)).toEqual([FRAMEWORK_2_1_0_AFFECTED_NOTICE]);
    });
    ...
    test('module', () => {
      ...
      // module-level match
      expect(noticesFilter.filter({ data: [NOTICE_FOR_APIGATEWAYV2], cliVersion, bootstrappedEnvironments: [], outDir: path.join(fixtures, 'experimental-module') }).map(f => f.notice)).toEqual([NOTICE_FOR_APIGATEWAYV2]);

      // no apigatewayv2 in the tree
      expect(noticesFilter.filter({ data: [NOTICE_FOR_APIGATEWAYV2], cliVersion, bootstrappedEnvironments: [], outDir: path.join(fixtures, 'built-with-2_12_0') }).map(f => f.notice)).toEqual([]);
      // module name mismatch: apigateway != apigatewayv2
      expect(noticesFilter.filter({ data: [NOTICE_FOR_APIGATEWAY], cliVersion, bootstrappedEnvironments: [], outDir: path.join(fixtures, 'experimental-module') }).map(f => f.notice)).toEqual([]);

      // construct-level match
      expect(noticesFilter.filter({ data: [NOTICE_FOR_APIGATEWAYV2_CFN_STAGE], cliVersion, bootstrappedEnvironments: [], outDir: path.join(fixtures, 'experimental-module') }).map(f => f.notice)).toEqual([NOTICE_FOR_APIGATEWAYV2_CFN_STAGE]);
    });
    ...
    test('module with pre-release version', () => {
      ...
      // module-level match
      expect(noticesFilter.filter({ data: [NOTICES_FOR_IDENTITY_POOL], cliVersion, bootstrappedEnvironments: [], outDir: path.join(fixtures, 'experimental-module-pre-release-semver') }).map(f => f.notice)).toEqual([NOTICES_FOR_IDENTITY_POOL]);
    });
