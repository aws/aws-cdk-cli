...
integTest('deploy with notification ARN as prop and flag', withDefaultFixture(async (fixture) => {
  ...
  const topic2Name = `${fixture.stackNamePrefix}-topic2`;

  const topic1Arn = await fixture.aws.temporaryTopic(topic1Name);
  const topic2Arn = await fixture.aws.temporaryTopic(topic2Name);
