...
export async function makeConfig(): Promise<CliConfig> {
  return {
    globalOptions: {
      ...
      'version-reporting': { type: 'boolean', desc: 'Disable CLI telemetry and do not include the "AWS::CDK::Metadata" resource in synthesized templates (enabled by default)', default: undefined, negativeAlias: 'disable-telemetry' },
      ...
      // 'disable-telemetry': { type: 'boolean', desc: 'Disable CLI telemetry and do not include the "AWS::CDK::Metadata" resource in synthesized templates (enabled by default)', default: undefined },
    },
    commands: {
      'list': {
        arg: {
