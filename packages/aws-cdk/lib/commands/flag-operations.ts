...
import { Toolkit, CdkAppMultiContext, MemoryContext } from '@aws-cdk/toolkit-lib';
import * as path from 'path';
import { StackSelectionStrategy } from '../api';
import * as fs from 'fs-extra';
import { DiffMethod } from '@aws-cdk/toolkit-lib';
...
async function prototypeChanges(
    ...
    for (const stack of allStacks) {
        const path = stack.templateFullPath;
        await toolkit.diff(modifiedCx, {
            method: DiffMethod.LocalFile(path),
            stacks: {
...
async function promptUser(
    ...
    return await ioHelper.requestResponse({
        time: new Date(),
        level: 'info',
...
async function modifyValues(flagName: string, value: string, ioHelper: IoHelper): Promise<void> {
    ...
    ioHelper.defaults.info(`Setting flag '${flagName}' to: ${boolValue}`);
