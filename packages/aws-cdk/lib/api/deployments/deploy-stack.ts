...
import { IO, SPAN, type IoHelper } from '../../../../@aws-cdk/tmp-toolkit-helpers/src/api/io/private';
...
class FullCloudFormationDeployment {
  ...
  private async monitorDeployment(startTime: Date, expectedChanges: number | undefined): Promise<SuccessfulDeployStackResult> {
    ...
    IO.DEFAULT_TOOLKIT_DEBUG.msg(format('Stack %s has completed updating', this.stackName));
    return {
      type: 'did-deploy-stack',
      noOp: false,
