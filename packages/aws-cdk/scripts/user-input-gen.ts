...
import { renderYargs, renderUserInputType, renderUserInputFuncs, renderJsonSchema } from '@aws-cdk/user-input-gen';
...
async function main() {
  ...
  fs.mkdirpSync('./schema');
  fs.writeFileSync('./schema/cdk-config.schema.json', renderJsonSchema(config));
}
